...
void solve() {
  ...
  vvi G(n);
  for (auto &u:F) u--;
  for (int i=0;i<n;i++) G[F[i]].eb(i);
  int ret = 0;
  // Check max from cycles.
  {
    vi vis(n);
    function<void(int,int)> dfs = [&](int u, int x) {
      vis[u] = x;
      if (!vis[F[u]]) dfs(F[u],x+1);
      if (vis[F[u]] < x) ckmax(ret,x-vis[F[u]]+1);
      vis[u] = n;
    };
    for (int i=0;i<n;i++) if (!vis[i]) dfs(i,i+1);
  }
  // Check for reciprocity segments
  {
    vi vis(n);
    ...
    int chains = 0;
    for (int i=0;i<n;i++) if (!vis[i] && i==F[F[i]]) {
      vis[i] = vis[F[i]] = 1;
      int a = dfs(i);
      int b = dfs(F[i]);
      chains += a+b;
