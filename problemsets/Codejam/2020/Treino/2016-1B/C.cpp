...
int bpm(int M, int N) {
    ...
    for (int i = 0; i < M; i++) if (sz(adj[i]) && par[i]==-1) ++id, ret += augment(i);
    return ret;
}
...
void solve() {
  int n; re(n);
  map<string,int> A, B;
  vector<vs> I(n,vs(2)); re(I);
  for (int i=0;i<n;i++) {
    if (!A.count(I[i][0])) { int u = sz(A); A[I[i][0]] = u; }
    if (!B.count(I[i][1])) { int v = sz(B); B[I[i][1]] = v; }
  }
  int M = sz(A), N = sz(B);
  ...
  int c = bpm(M,N);
  for (int i=0;i<M+N;i++) if (par[i]==-1) c++;
  ps(n-c);
}
