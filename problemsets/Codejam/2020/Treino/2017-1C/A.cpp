...
void solve() {
  int n, k; re(n,k);
  vll R(n), H(n);
  for (int i=0;i<n;i++) re(R[i],H[i]);
  double ret = -1;
  vi ord(n);
  for (int i=0;i<n;i++) ord[i] = i;
  sort(all(ord),[&](int u, int v){ return H[u]*R[u]>H[v]*R[v]; });
  for (int i = 0; i < n; i++) {
    double cur = M_PI*H[i]*R[i]*2. + M_PI*R[i]*R[i];
    int r = R[i];
    int j = 1, i1 = 0;
    while (i1 < n && j < k) {
      if (ord[i1]==i) { i1++; continue; }
      if (R[ord[i1]]>r) { i1++; continue; }
      cur += M_PI*H[ord[i1]]*R[ord[i1]]*2.;
      j++; i1++;
    }
    if (j==k && cur > ret) ret = cur;
  }
  ps(ret);
}
