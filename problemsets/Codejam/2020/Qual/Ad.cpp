...
void solve() {
  ...
  Num A(a), B(b);
  Num y = gcd(A,B);
  plain.eb(string(A/y)); plain.eb(string(y));
  A = B/y; plain.eb(string(A));
  for (int i = 0; i < l-2; i++) {
    cin >> b;
    A = Num(b)/A; plain.eb(string(A));
  }
...
int main() {
  ...
  for (int i = 1; i <= t; i++) {
    string a, b;
    cin >> a; int c; cin >> c; cin >> a;
    vector<string> ret;
    for (int i = 0; i < 25; i++) {
      cin >> b; cout << string(Num(a)*Num(b)) << " "; a = b;
      ret.eb(b);
    }
  sort(all(ret), [](string &a, string &b){ return a.size() != b.size() ? a.size() > b.size() : a > b;});
