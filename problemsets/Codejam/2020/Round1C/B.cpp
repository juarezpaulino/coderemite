...
void solve() {
  ...
  map<char,int> m;
  for (auto &u:R) m[u[0]]++;
  auto L = vector<pair<char,int>>(all(m));
  sort(all(L),[&](auto &u, auto &v){ return u.se>v.se; });
  for (int i=1;i<10;i++) ret[i] = L[i-1].fi;
  for (auto &u:R) for (auto &v:u) if (!m.count(v)) {
    ret[0] = v;
    break;
  }  
  ps(ret);
