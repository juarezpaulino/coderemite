...
void solve() {
  int x, y; re(x,y);
  if ((x%2)^(y%2)) { ps("IMPOSSIBLE"); return; }
  string ret = "";
  int mini = 1E9;
  string dir = "NESW";
  int k=0,l=0;
  ...
  for (int i=0;i<2;i++) for (int j=0;j<2;j++) {
    string a="", b="";
    int v;
    if (i==0) { v = x; while (v) { a+=i%2?"+":"0"; v/=2; } }
    else { int u = 32-__builtin_clz(x); v = (1<<u)-x; while (v) { a+=i%2?"-":"0"; v/=2; } while (sz(a)<u) a = "0"+a; a = "+"+a; }
    if (j==0) { v = y; while (v) { a+=i%2?"+":"0"; v/=2; } }
    else { int u = 32-__builtin_clz(x); v = (1<<u)-x; while (v) { a+=i%2?"-":"0"; v/=2; } while (sz(a)<u) a = "0"+a; a = "+"+a; }
  }
}
