...
//	freopen("D-large.in","r",stdin);freopen("D-large.ans","w",stdout);
    ...
    for (int tc = 1; tc <= TC; tc++) {
        ...
        // Try every possible last element. For each possibility build up the
        // adjacency weight graph and sum up the hamiltonian cycle excluding
        // the fixed last node.
        int ret = INF;
        N = K;
        FOR(i,0,N) {
            int r = 0;
            CLR(adj,0);
            FOR(j,0,N) FOR(k,0,N) FOR(b,0,NB)
                if (j == i) adj[j][k] += (b < NB-1 && B[b][j]!=B[b+1][k]);
                else adj[j][k] += B[b][j]!=B[b][k];
            CLR(DP,-1); r += solve((1<<i), i, i);
            ret = min(ret, r);
        }
