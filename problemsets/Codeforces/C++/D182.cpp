...
int main() {
  ...
  string a, b; re(a,b);
  int na = sz(a), nb = sz(b);
  vi pa = prefix_function(a);
  vi pb = prefix_function(b);
  int ka = na - pa[na-1]; ka = na%ka==0 ? ka : na;
  int kb = nb - pb[nb-1]; kb = nb%kb==0 ? kb : nb;
  if (ka != kb || a.substr(0,ka) != b.substr(0,kb)) { pr(0); return 0; }
  int c = na/ka, d = nb/kb;
  if (d < c) swap(c,d);
  ll ret = 1;
  int l = sqrt(d);
