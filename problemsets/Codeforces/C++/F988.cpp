...
main() {
  ...
  for (int i = 0; i < m; i++) {
    int x, p; re(x, p);
    cost[i] = p;
    if (umbrela[x]<0 || cost[umbrela[x]] > p)
      umbrela[x] = i;
  }
  ...
  for (int k = 0; k < a; k++) {
    ...
    for (int i = 0; i <= m; i++) {
      ...
      if (!rainy[k]) ckmin(new_dp[m],dp[i]);
      if (i < m) ckmin(new_dp[i], dp[i]+cost[i]);
      if (umbrela[k]>=0) ckmin(new_dp[umbrela[k]], dp[i]+cost[umbrela[k]]);
    }
    dp = move(new_dp);
  }
