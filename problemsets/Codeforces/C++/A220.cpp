...
main() {
  ...
  vi a(n); re(a);
  int u = -1, v = -1;
  for (int i = 1; i < n; i++) if (a[i] < a[i-1]) { u = i-1; break; }
  for (int i = n-2; i >= 0; i--) if (a[i] > a[i+1]) { v = i+1; break; }
  while (u > 0 && a[u-1]==a[u]) u--;
  while (v > 0 && v < n-1 && a[v+1]==a[v]) v++;
  if (u != -1) swap(a[u], a[v]);
  ps(is_sorted(all(a))?"YES":"NO");
}