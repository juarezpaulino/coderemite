...
main() {
  ...
  vi init(n), goal(n); re(init, goal);
  vi vis(n);
  vi ret;
  function<void(int,int,int,int)> dfs = [&](int u, int cur_l, int l_odd, int l_even) {
    vis[u] = 1;
    int cur = init[u]^(cur_l%2 ? l_odd%2 : l_even%2);
    if (cur != goal[u]) {
      ret.eb(u+1);
      cur_l%2 ? l_odd++ : l_even++;
    }
    for (auto &v:adj[u]) if (!vis[v])
      dfs(v,cur_l+1,l_odd,l_even);
  };
  dfs(0,0,0,0);
  ps(sz(ret));
  for (auto &u:ret) ps(u);
