...
int main() {
  ...
  while (q--) {
    string a; char b; re(a,b);
    m.emplace(mp(b,a[0]));
  }
  int dp[10][10]; ms(dp,-1);
  function<int(int,char)> go = [&](int k, char c) {
    int &ret = dp[k][c-'a'];
    if (ret != -1) return ret;
    if (k == n-1) return 1;
    ret = 0;
    auto b = m.lower_bound(c), e = m.upper_bound(c);
    for (auto u = b; u != e; u++) ret += go(k+1, u->s);
    return ret;
  };
  ps(go(0,'a'));
}