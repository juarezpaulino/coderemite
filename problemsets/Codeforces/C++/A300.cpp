...
main() {
  ...
  vi a(n); re(a);
  vi pos, neg, zer;
  for (auto &u:a) u>0?pos.eb(u):u<0?neg.eb(u):zer.eb(u);
  if (!sz(pos)) pos.eb(neg.back()), pos.eb(*(end(neg)-2)), neg.pop_back(), neg.pop_back();
  ps(1,neg[0]);
  pr(sz(pos)); for (auto &u:pos) pr(' ',u); ps();
  pr(sz(zer)+sz(neg)-1);
  for (auto &u:zer) pr(' ',u);
  for (int i=1;i<sz(neg);i++) pr(' ',neg[i]); ps();
}