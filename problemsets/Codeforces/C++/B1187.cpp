...
main() {
  ...
  int m; re(m);
  vs t(m); re(t);
  map<pair<char,int>, int> pos;
  vi cnt(30);
  for (int i=0;i<n;i++) pos[mp(s[i],++cnt[s[i]-'a'])] = i;
  for (auto &u:t) {
    vi cur_cnt(30);
    for (auto &v:u) cur_cnt[v-'a']++;
    int mx = -INF;
    for (int i=0;i<30;i++) if (cur_cnt[i]) ckmax(mx, pos[mp(i+'a',cur_cnt[i])]);
    ps(mx+1);
  }
}