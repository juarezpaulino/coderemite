...
main() {
  ...
  vi c(k*n); re(c);
  map<int,int> C; for (auto &u:c) C[u]++;
  
  vi f(n); re(f);
  map<int,int> F; for (auto &u:f) F[u]++;

  vi h(k); re(h);

  ms(dp,-1);
  function<int(int,int)> go = [&](int p, int c) {
    if (dp[p][c] >= 0) return dp[p][c];
    int &ret = dp[p][c];
    int l = min(k,c);
    if (c==0) return ret=0;
    if (p==1) return ret=h[l-1];
    ret = 0;
    for (int j = 0; j < l; j++) ckmax(ret, h[j]+go(p-1, c-j-1));
    return ret;
  };

  int ret = 0;
  for (auto &u:F) ret += go(u.se, C[u.fi]);
  ps(ret);
}