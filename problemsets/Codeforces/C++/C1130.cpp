...
int main() {
  ...
  int n, r1, c1, r2, c2; re(n,r1,c1,r2,c2);
  vs a(n+1); FOR(i,1,n+1) re(a[i]), a[i] = "x"+a[i];
  int dy[] = { -1, 0, 1, 0 };
  int dx[] = { 0, -1, 0, 1 };
  int color;
  ...
  color = 'a'; dfs(r1,c1);
  if (a[r2][c2]=='a') { pr(0); return 0; }
  color = 'b'; dfs(r2,c2);
  int ret = MOD;
  FOR(i,1,n+1) FOR(j,1,n+1) if (a[i][j]=='a')
  FOR(k,1,n+1) FOR(l,1,n+1) if (a[k][l]=='b')
    ret = min(ret, sqr(i-k)+sqr(j-l));
  pr(ret);
}
