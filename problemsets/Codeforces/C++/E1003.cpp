...
main() {
  ...
  int n, d, k; re(n, d, k);
  if (d >= n || (n > 2 && k < 2)) { ps("NO"); return 0; }

  priority_queue<pii,vpi,greater<pii>> q;
  vpi edges;
  vi deg(n);
  for (int i = 0; i < d; i++) edges.eb(mp(i,i+1)), q.push(mp(i,max(i,d-i))), deg[i]++, deg[i+1]++;
  q.push(mp(d,d));

  int ne = d;
  while (ne < n-1 && !q.empty()) {
    int u, v; tie(u,v) = q.top(); q.pop();
    if (deg[u]>=k || v>=d) continue;
    ne++;
    edges.eb(mp(u,ne));
    deg[u]++; deg[ne]++;
    q.push(mp(u,v));
  ...
  if (ne < n-1) ps("NO");
    ...
    ps("YES");
    for (auto &e:edges) ps(e.fi+1, e.se+1);
  }
}