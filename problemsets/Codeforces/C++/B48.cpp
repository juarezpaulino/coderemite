...
main() {
  ...
  auto getV = [&](int yl, int xl, int yr, int xr) {
    int ret = a[yr][xr];
    if (yl) ret -= a[yl-1][xr];
    if (xl) ret -= a[yr][xl-1];
    if (yl && xl) ret += a[yl-1][xl-1];
    return ret;
  };
  dbg(a);
  int ret = INF;
  ...
  for (int i=0;i<n;i++) for (int j=0;j<n;j++) {
    if (i+u-1 >= n || j+v-1 >= m) continue;
    ckmin(ret,getV(i,j,i+u-1,j+v-1));
  }
  for (int i=0;i<n;i++) for (int j=0;j<n;j++) {
    if (i+v-1 >= n || j+u-1 >= m) continue;
    ckmin(ret,getV(i,j,i+v-1,j+u-1));
  }
  ps(ret);
}