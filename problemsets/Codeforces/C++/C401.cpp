...
string repeat_string(int c, string p) {
  string ret = "";
  for (; c; c/=2, p+=p) if (c&1) ret += p;
  return ret;
}
...
int main() {
  ...
  int n, m; re(n,m);
  if (n > m+1 || m > 2*(n+1)) { ps(-1); return 0; }
  if (n == m+1) ps("0" + repeat_string(m,"10"));
  else if (m > 2*n) ps(repeat_string(n,"110") + (m == 2*n+1 ? "1" : "11"));
  else ps(repeat_string(m-n,"110") + repeat_string(2*n-m,"10"));
}