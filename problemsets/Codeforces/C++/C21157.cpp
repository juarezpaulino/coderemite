...
main() {
  ...
  int last = -1;
  int i = 0, j = n-1;
  // Solves always for left
  while (i<=j && (a[i]>last || a[j]>last)) {
    if (a[i]>last && (a[i]<=a[j] || a[j]<=last)) b += "L", last = a[i++];
    else b += "R", last = a[j--];
  }
  last = -1, i = 0, j = n-1;
  // Solves always for right
  while (i<=j && (a[i]>last || a[j]>last)) {
    if (a[j]>last && (a[j]<=a[i] || a[i]<=last)) c += "R", last = a[j--];
    else c += "L", last = a[i++];
  }
  if (sz(c) > sz(b)) b = c;
  ps(sz(b));
