...
int main() {
  ...
  vpi a(n); FOR(i,0,n) re(a[i].f), a[i].s = i;
  vpi b(m); FOR(i,0,m) re(b[i].f), b[i].s = i;
  sort(all(a)); sort(all(b));
  vpi ret;
  int i = 0, j = 0;
  while (j < m) {
    while (i < n && b[j].f > a[i].f+y) i++;
    if (i < n && a[i].f-x <= b[j].f) ret.eb(mp(a[i].s+1,b[j].s+1)), i++;
    j++;
  }
  ps(sz(ret));
  FORE(u,ret) ps(u.f,u.s);
}