...
main() {
  ...
  auto cross = [&](pii u, pii v) {
    return u.fi*(ll)v.se - u.se*(ll)v.fi;
  };
  auto picks = [&](int i, int j) {
    int l = (j+n-i)%n+1;
    // First compute area of polygon
    ll ret = 0;
    for (int k=0;k<l;k++) {
      int u = (i+k)%n;
      int v = k==l-1?i:(u+1)%n;
    ...
    // Now find number of boundary vertices.
    ll b = 0;
    for (int k=0;k<l;k++) {
      int u = (i+k)%n;
      int v = k==l-1?i:(u+1)%n;
      dbg(u,v);
      b += __gcd(abs(a[u].fi-a[v].fi),abs(a[u].se-a[v].se));
      u = v;
    }
    dbg(ret,b);
    ret-=b;
  ...
  ld ret = 0;
  for (int i=0;i<n;i++) for (int j=1;j<min(n-1,50);j++) {
    int k = (i+j)%n;
    if (k < i && n < 50) break;
    int l1 = (k+n-i-1)%n;
    int l2 = (n-2-l1);
    ...
    if ((k+n-i)%n+1 > 2) {
      ll p = picks(i,k);
      dbg(i,k,p);
      ret += p * 1/4. * fast_pow(.5,l1);
    }
    if (l2 < 50 && (i+n-k)%n+1 > 2) ret += picks(k,i) * 1/4. * fast_pow(.5,l2);
  }
  ps(ret);
}