...
typedef map<ll, int> prime_map;
void squeeze(prime_map &M, ll &n, int p) { for (; n % p == 0; n /= p) M[p]++; }
prime_map factor(ll n) {
    prime_map M;
    if (n<2) return M;
    squeeze(M, n, 2); squeeze(M, n, 3);
    int maxP=(int)sqrt(n)+2;
    for (int p=5; n!=1 && p<maxP; p+=6) {
        squeeze(M, n, p);
        squeeze(M, n, p+2);
    }
    if (n>1) M[n]++;
...
main() {
  ...
  prime_map m = factor(d);
  ll ret = 1;
  for (auto &u:m) ret *= u.se+1;
  pr(ret);
}