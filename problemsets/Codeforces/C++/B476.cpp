...
main() {
  ...
  string a, b; re(a,b);
  int n = sz(b);
  vector<vd> dp(15,vd(30,-1));
  int t = 15;
  for (auto &u:a) t+=(u=='+')?1:-1;
  function<double(int,int)> go = [&](int k, int u) {
    if (k==n) return u==t?1.:0.;
    double &ret = dp[k][u];
    if (ret > 1E-10) return ret;
    if (b[k]=='+') ret = go(k+1,u+1);
    else if (b[k]=='-') ret = go(k+1,u-1);
    else ret = .5*go(k+1,u-1) + .5*go(k+1,u+1);
    return ret;
  };
  pr(go(0,15));
}