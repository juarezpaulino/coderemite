...
main() {
  ...
  if (count(all(a),'0') > 1) {
    int p1 = a.rfind('0');
    int p2 = a.rfind('0', p1-1);
    ckmin(ret, 2*n-3-p1-p2);
  }
  // 25
  a = b;
  if (a.find('2')!=-1 && a.find('5')!=-1) {
    int p1 = a.rfind('2');
    int p2 = a.rfind('5');
    if (p2 < p1) { a.erase(p1,1); a.erase(p2,1); }
    else { a.erase(p2,1); a.erase(p1,1); }
    int nxt_nz = 0;
    for (; nxt_nz < n-2 && a[nxt_nz]=='0'; nxt_nz++);
    if (nxt_nz!=n-2 || n==2) 
      ckmin(ret, 2*n-3-p1-p2 + (p1>p2) + nxt_nz);
  }
  // 50
  a = b;
  if (a.find('5')!=-1 && a.find('0')!=-1) {
    int p1 = a.rfind('5');
    int p2 = a.rfind('0');
    if (p2 < p1) { a.erase(p1,1); a.erase(p2,1); }
    else { a.erase(p2,1); a.erase(p1,1); }
