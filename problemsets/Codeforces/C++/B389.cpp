...
main() {
  ...
  int n; re(n);
  vs a(n+2, str(n+2,'.'));
  for (int i=0;i<n;i++) for (int j=1;j<=n;j++) re(a[i][j]);
  bool ok = 1;
  for (int i=0;i<n;i++) for (int j=1;j<=n;j++) if (a[i][j]=='#') {
    int c = 0;
    int y = i+1, x = j;
    for (auto &u:vpi({{0,0},{-1,0},{0,-1},{0,1},{1,0}})) {
      int ny=y+u.fi, nx=x+u.se;
      if (a[ny][nx]=='#') a[ny][nx]='.', c++;
    } 
    ok &= c==5;
  }
  ps(ok?"YES":"NO");
