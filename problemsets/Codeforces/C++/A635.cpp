...
main() {
  ...
  int ret = 0;
  for (int y1=0;y1<r;y1++) for (int y2=y1;y2<r;y2++) for (int x1=0;x1<c;x1++) for (int x2=x1;x2<c;x2++) {
    int u = 0;
    for (int i=y1;i<=y2;i++) for (int j=x1;j<=x2;j++) u += a[i][j];
    ret += u>=k;
  }
  ps(ret);
}