...
int main() {
  ...
  int n, g, k1, k2; re(n);
  set<pqi> U;
  re(k1); qi a; FOR(i,0,k1) re(g), a.push(g);
  re(k2); qi b; FOR(i,0,k2) re(g), b.push(g);
  U.insert(mp(a,b));
  g = 0;
  while (1) {
    g++;
    ...
    if (U.count(mp(a,b)) || a.empty() || b.empty()) break;
    U.insert(mp(a,b));
  }
  if (a.empty()) ps(g,2);
  else if (b.empty()) ps(g,1);
  else ps(-1);
}