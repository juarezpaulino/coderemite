...
int main() {
  ...
  int ax, ay, bx, by, cx, cy; re(ax,ay,bx,by,cx,cy);
  auto dist = [](int ax, int ay, int bx, int by) {
    return sqr(1LL*ax-bx)+sqr(1LL*ay-by);
  };
  auto colinear = [](int ax, int ay, int bx, int by, int cx, int cy) {
    return (1LL*ax-bx)*(cy-by) == (1LL*ay-by)*(cx-bx);
  };
  pr(dist(ax,ay,bx,by) != dist(bx,by,cx,cy) || colinear(ax,ay,bx,by,cx,cy) ? "No" : "Yes");
}
