...
// OUTPUT
template<class A> void pr(A x) { cout << fixed << setprecision(10) << ts(x); }
...
// State.fi = remainder
// State.se = max dig as prefix
// First dimension - minimum number of ways to reach <= 0.
// Second dimension - remainder after reduction.
map<pli,pli> dp;

pli solve(ll u, int mx_dig) {
  if (u < 10) {
    if (u==0) return mp(0,0);
    u -= max((ll)mx_dig,u);
    return mp(1,-u);
  }
  if (dp.count(mp(u,mx_dig))) return dp[mp(u,mx_dig)];

  pli &ret = dp[mp(u,mx_dig)];
  ret = mp(0,0);
  ll step = 1;
  while (step <= u/10) step *= 10;
  while (1) {
    int cur_dig = u/step;
    int nx_dig = max(mx_dig, cur_dig);
    pli reduce = solve(u%step, nx_dig);
    ret.fi += reduce.fi;
    u -= u%step;
    u -= (reduce.se > 0) ? reduce.se : (ret.fi+=nx_dig>0, nx_dig);
    if (cur_dig == 0) break;
  }
  ret.se = -u;
  return ret;
