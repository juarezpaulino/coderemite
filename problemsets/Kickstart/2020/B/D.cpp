...
void solve() {
  int W, H, L, U, R, D;
  re(W,H,L,U,R,D);
  for (int i=0;i<H+10;i++) for (int j=0;j<W+10;j++) dp[i][j] = -1.;  
  function<double(int,int)> go = [&](int y, int x) {
    double &ret = dp[y][x];
    if (ret > -EPS) return ret;
    if (y==H && x==W) return ret = 1.;
    if (U<=y && y<=D && L<=x && x<=R) return ret = 0.;
    ret = 0.;
    int c = (y<H) + (x<W);
    if (y<H) ret += 1./c * go(y+1,x);
    if (x<W) ret += 1./c * go(y,x+1);
    return ret;
  };
  ps(go(1,1));
}
