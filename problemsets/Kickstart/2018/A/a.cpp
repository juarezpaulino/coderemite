...
const int MAXDIG = 20;     // Max non-decimal digits in a number.
...
void solve() {
  ...
  string a; cin >> a;
  int n = sz(a);
  int p = find_if(all(a), [](char c) { return !iseven(c); }) - begin(a);
  if (p == n) { cout << "0" << endl; return; }
  
  string x = a.substr(0,p)+ string(1,a[p]-1) + string(n-p-1,'8');
  
  while (p >= 0 && (a[p]=='8' || a[p]=='9')) p--;
  string y = p < 0 ?
             "2"+string(n,'0') :
             a.substr(0,p) + string(1, a[p] + (iseven(a[p])?2:1)) + string(n-p-1, '0');
  Num X = Num(a) - Num(x);
  Num Y = Num(y) - Num(a);
  
  cout << (X < Y ? string(X) : string(Y)) << endl;
}
