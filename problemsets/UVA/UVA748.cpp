...
const int MAXDIG = 200;     // Max non-decimal digits in a number.
const int MAXFRAC = 150;      // Max precision required for decimals.
...
struct Num {
    ...
    operator string() const {
        ostringstream s;
        if (v[n]<0) s << '-';
        if (FRAC && n <= FRAC) { s << "."; s.width(NBASE); s.fill('0'); }
        s << abs(v[n]);
        for (int i = n - 1; i > 0; i--) {
            if (i==FRAC) s << '.';
