...
int check(int np, int v, int mx, int sum) {
    ...
    if ((ret>>16) != (v?id:++id))
        for (ret = id<<16, i = prod[np][v].lo; i <= min(prod[np][v].hi, mx); i++)
            if (check(np, v+1, i, sum-i)) ret|=1, prod[np][v].mhi=max(prod[np][v].mhi,i), prod[np][v].mlo=min(prod[np][v].mlo,i);
    return ret&1;
}
...
int main() {
    ...
    while (scanf("%d", &p), p) {
        ...
        for (k = 0; maxlo = 0, minhi = 100, k < q; k++) {
            scanf("%s %s", op, str);
            for (i = 0; x = y = 0, i < p; minhi=min(minhi,x), maxlo=max(maxlo,y), i++) for (j = 0; j < n[i]; j++)
