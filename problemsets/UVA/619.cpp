...
////////////////////////////////////////////////////////////////////////////////
// Big integer with fractional part (division with frac not working yet)
//
////////////////////////////////////////////////////////////////////////////////
using namespace std;
////////////////////////////////////////////////////////////////////////////////
// Big Number
// - Num(char *s): s = (DIG+)[.DIG*]
// * Frac part still not working with:
// - division (but working for div2)
// * Be aware with .root() method since you have to increase MAXDIG value.
// By some yet unknown reason you must keep MAXDIG something bigger than MAXFRAC
////////////////////////////////////////////////////////////////////////////////
// Setable parameters.
const int MAXDIG = 35;     // Max non-decimal digits in a number.
    ...
    while (scanf("%s", S) != EOF) {
        ...
        if (*S >= '0' && *S <= '9') {
            ...
            if (A == 0) ret = "a";
                ...
                int v;
                sscanf(string(A%26).c_str(), "%d", &v);
                ret = ret + string(1, (v-1+'a'));
                A /= 26;
            }
            reverse(ret.begin(), ret.end());
            printf("%-22s%s\n", ret.c_str(), format(S).c_str());
        }
        else {
            Num ret = 0;
            for (int i = 0; S[i]; i++) {
