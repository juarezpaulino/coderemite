...
void DFS(int k, string S) {
    if (k == 9) {
        bool ok = true;
        FOR(i,0,9) if (V[i]!=0) { ok = false; break; }
        if (ok && (ret=="" || S<ret)) ret = S;
        return;
    }
    DFS(k+1, S);
    string S2 = S;
    FOR(i,0,3) {
        FOR(j,0,SI(M[k])) V[M[k][j]-'0'] = (V[M[k][j]-'0']+1)%4;
        S2 += string(1, k+'1') + " ";
        DFS(k+1, S2);
    }
    FOR(i,0,3) FOR(j,0,SI(M[k])) V[M[k][j]-'0'] = (V[M[k][j]-'0']+3)%4;
}
...
int main() {
    ...
    FOR(i,0,9) scanf("%d", V+i), V[i]/=3, V[i]%=4;

    ret = "";
    DFS(0,"");

    printf("%s\n", (ret.substr(0,SI(ret)-1)).c_str());
