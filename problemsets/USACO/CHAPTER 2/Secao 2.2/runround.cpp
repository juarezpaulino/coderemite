...
inline string ITOA(unsigned int a) { char c[500]; sprintf(c,"%u",(a)); return string(c); }
...
bool is_runaround(int V) {
    string S = ITOA(V);
    int p = 0, N = SI(S), c = 0;
    FOR(i,0,N) FOR(j,i+1,N) if (S[i]==S[j]) return false;
    char vis[11] = {0};
    while (!vis[p]) {
        vis[p] = 1; c++;
        p = (p+S[p]-'0')%N;
...
//    freopen("runround.in","r",stdin);freopen("runround.out","w",stdout);
    ...
    scanf("%u", &M);
    for (M = M+1; ; M++) if (is_runaround(M)) break;
    printf("%u\n", M);
