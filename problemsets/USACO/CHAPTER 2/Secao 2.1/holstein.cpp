...
int main() {
    ...
    scanf("%d", &M);
    FOR(i,0,M) FOR(j,0,N) scanf("%d", &G[i][j]);

    FOR(i,0,1<<M) {
        FOR(j,0,N) R[j] = 0;
        FOR(k,0,M) if (i&(1<<k)) FOR(j,0,N) R[j] += G[k][j];
        bool ok = true;
        FOR(j,0,N) if (R[j] < V[j]) { ok = false; break; }
        if (ok) {
            VI C;
            FOR(k,0,M) if (i&(1<<k)) C.PB(k);
            if (SI(RET)==0 || SI(C) < SI(RET)) RET = C;
        }
    }

    printf("%d", SI(RET));
    FOR(i,0,SI(RET)) printf(" %d", RET[i]+1);
    putchar('\n');
