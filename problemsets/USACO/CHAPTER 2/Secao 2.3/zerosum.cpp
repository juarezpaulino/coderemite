...
inline string ITOA(int a) {
    char c[500]; sprintf(c,"%d",(a));
    return (a > 0) ? string(1,'+')+c : c;
}
...
void go(int s, int l, int k, string cur) {
    if (k > N) {
        if (s+l==0) {
            cur = cur+ITOA(l);
            cur = cur.substr(1);
            string r;
            FOR(i,0,SI(cur)) {
                if (i && isdigit(cur[i])&&isdigit(cur[i-1])) r += " ";
                r += cur[i];
            }
            RET.PB(r);
        }
    ...
    go(s+l,k,k+1,cur+ITOA(l));
    go(s+l,-k,k+1,cur+ITOA(l));
    if (l > 0) go(s,l*10+k,k+1,cur);
    else go(s,l*10-k,k+1,cur);
}
