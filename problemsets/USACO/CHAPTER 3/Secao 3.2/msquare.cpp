...
hash_map<VI,string> mapa;
VI GOAL(8);

VI A(const VI &V) {
    VI RET(8);
    FOR(i,0,8) RET[(i+4)%8] = V[i];
    return RET;
}

VI B(const VI &V) {
    VI RET(8);
    RET[0] = V[3]; FOR(i,1,4) RET[i] = V[i-1];
    RET[4] = V[7]; FOR(i,5,8) RET[i] = V[i-1];
    return RET;
}
...
void BFS() {
    ...
    while (!q.empty()) {
        ...
        if (U == GOAL) return;
        V = A(U);
        if (!mapa.count(V)) { q.push(V); mapa[V] = mapa[U]+"A"; }
        V = B(U);
        if (!mapa.count(V)) { q.push(V); mapa[V] = mapa[U]+"B"; }
        V = C(U);
        if (!mapa.count(V)) { q.push(V); mapa[V] = mapa[U]+"C"; }
    }
}
...
int main() {
    ...
    FOR(i,0,4) { int x; scanf("%d", &x); GOAL[i] = x-1; }
    for (int i = 7; i >= 4; i--) { int x; scanf("%d", &x); GOAL[i] = x-1; }

    BFS();

    string ret = mapa[GOAL];

    printf("%d\n", SI(ret));
    FOR(i,0,SI(ret)) printf((i%60==59) ? "%c\n" : "%c", ret[i]);
    putchar('\n');
