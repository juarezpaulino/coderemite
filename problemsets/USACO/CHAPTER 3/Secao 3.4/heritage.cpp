...
char IN[30], PRE[30];
int N;
char POS[256], LEFT[256], RIGHT[256];
char DISC[30];
int node;

void trace(char par, bool left) {
    char ch = PRE[node];
    int p = POS[ch];
    DISC[p] = 1;
    if (p > 0 && !DISC[p-1]) { node++; trace(ch, 1); }
    if (p < N-1 && !DISC[p+1]) { node++; trace(ch, 0); }
    if (par != -1) {
        if (left) LEFT[par] = ch;
        else RIGHT[par] = ch;
    }
...
void pos_order(char ch) {
    if (LEFT[ch] != -1) pos_order(LEFT[ch]);
    if (RIGHT[ch] != -1) pos_order(RIGHT[ch]);
    putchar(ch);
}
...
int main() {
    ...
    scanf("%s %s", IN, PRE); N = strlen(IN);
    for (int i = 0; i < N; i++) DISC[i] = 0;
    for (int i = 0; IN[i]; i++) { POS[IN[i]] = i; LEFT[IN[i]] = RIGHT[IN[i]] = -1; }
    node = 0; trace(-1, 0);
    pos_order(PRE[0]); putchar('\n');
