...
char HERD[4][5];
char DISC[4][4];
int last[5] = { 3, 3, 3, 4, 3 };
struct Cell { int y, x; char T; } SOL[16];

int C;

void DFS(int k) {
    if (k == 16) {
        if (!C) FOR(i,0,16) printf("%c %d %d\n", SOL[i].T, SOL[i].y+1, SOL[i].x+1);
        C++;
        return;
    }
    FOR(m,0,5) {
        ...
        FOR(i,0,4) FOR(j,0,4) if (!DISC[i][j]) {
            bool ok = true;
            FOR(o,-1,2) FOR(p,-1,2) {
                if (i+o >= 0 && i+o < 4 && j+p >= 0 && j+p < 4 && HERD[i+o][j+p]=='A'+m) {
                    ok = false;
                    break;
                }
            }
            if (ok) {
                char tmp = HERD[i][j];
                HERD[i][j] = 'A' + m;
                SOL[k].T = 'A' + m; SOL[k].y = i; SOL[k].x = j;
                last[m]--;
                DISC[i][j] = 1;
                DFS(k+1);
                DISC[i][j] = 0;
