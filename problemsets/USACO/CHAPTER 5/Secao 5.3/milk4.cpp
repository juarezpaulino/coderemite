...
inline bool check(bool *M, int p) {
    FOR(i,P[p],Q+1) M[i] = M[i] | M[i-P[p]];
    if (M[Q]) return true;
    return false;
}

void DFS(bool *CUR, int k, int p) {
    bool M[20001];
    if (k+1 >= B) return;
    FOR(i,p+1,NP) {
        memcpy(M, CUR, sizeof(M));
        if (check(M, i)) {
            memcpy(U, USED, sizeof(U)); U[i] = 1;
            B = k+1;
            return;
        }
        USED[i] = 1;
...
int main() {
    ...
    VI ret;
    FOR(i,0,NP) if (U[i]) ret.PB(P[i]);
    printf("%d", B);
    FOR(i,0,SI(ret)) printf(" %d", ret[i]);
    putchar('\n');
