...
// Setable parameters.
const int MAXDIG = 50;     // Max non-decimal digits in a number.
...
int main() {
    ...
    FOR(i,0,N+1) {
        DP[i] = 1;
        FOR(j,0,i) if (V[j] > V[i] && DP[j]+1 > DP[i]) DP[i] = DP[j]+1;
    }
    FOR(i,0,N+1) {
        if (DP[i]==1) S[i] = 1;
        FOR(j,0,i) if (V[j] > V[i] && DP[i]==DP[j]+1) S[i] += S[j];
        int j;
        for (j = 0; j < i && (DP[j]!=DP[i] || V[j]!=V[i]); j++);
        if (j != i) S[i]-=S[j];
    }

    printf("%d %s\n", DP[N]-1, string(S[N]).c_str());
