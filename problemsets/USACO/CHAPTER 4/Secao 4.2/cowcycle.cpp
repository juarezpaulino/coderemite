...
void compute() {
    if (CF[F-1]*CR[R-1] < 3*CF[0]*CR[0]) return;
    double V[50]; int nv = 0;
    FOR(i,0,F) FOR(j,0,R) V[nv++] = CF[i]/(double)CR[j];
    sort(V, V+nv);
    FOR(i,1,nv) V[i-1] = V[i]-V[i-1];
    nv--;
    double m = 0., v = 0.;
    FOR(i,0,nv) m += V[i]; m /= nv;
    FOR(i,0,nv) v += (V[i]-m) * (V[i]-m); v /= nv;
    if (cmp(v,B) >= 0) return;
    B = v;
    FOR(i,0,F) BF[i] = CF[i];
    FOR(i,0,R) BR[i] = CR[i];
...
void GOR(int k, int S) {
    if (k == R) { compute(); return; }
    FOR(i,S,R2+1) {
        CR[k] = i;
        GOR(k+1, i+1);
    }
...
void GOF(int k, int S) {
    if (k == F) { GOR(0, R1); return; }
    FOR(i,S,F2+1) {
        CF[k] = i;
        GOF(k+1, i+1);
    }
...
int main() {
    ...
    FOR(i,0,F) printf((i==F-1) ? "%d\n" : "%d ", BF[i]);
    FOR(i,0,R) printf((i==R-1) ? "%d\n" : "%d ", BR[i]);
