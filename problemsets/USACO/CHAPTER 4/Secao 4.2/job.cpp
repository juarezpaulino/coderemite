...
int main() {
    ...
    scanf("%d %d %d", &N, &M1, &M2);
    FOR(i,0,M1) scanf("%d", A+i), T1[i] = A[i];
    FOR(i,0,M2) scanf("%d", B+i), T2[i] = B[i];

    FOR(i,0,N) {
        t = 0;
        FOR(j,1,M1) if (T1[j] < T1[t]) t = j;
        J1[i] = T1[t];
        T1[t] += A[t];

        t = 0;
        FOR(j,1,M2) if (T2[j] < T2[t]) t = j;
        J2[N-1-i] = T2[t];
        T2[t] += B[t];
    }

    int t1 = 0, t2 = 0;
    FOR(i,0,N) t1 = max(t1,J1[i]), t2 = max(t2,J1[i]+J2[i]);
    printf("%d %d\n", t1, t2);
